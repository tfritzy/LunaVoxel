...
import { ModifyBlocks } from "./modify_blocks_reducer.ts";
export { ModifyBlocks };
...
const REMOTE_MODULE = {
  ...
  reducers: {
    ...
    ModifyBlocks: {
      reducerName: "ModifyBlocks",
      argsType: ModifyBlocks.getTypeScriptAlgebraicType(),
    },
    PokeProject: {
...
| { name: "ModifyBlocks", args: ModifyBlocks }
...
export class RemoteReducers {
  ...
  onCommitSelectionMove(callback: (ctx: ReducerEventContext, projectId: string, offset: Vector3) => void) {
    this.connection.onReducer("CommitSelectionMove", callback);
  }

  removeOnCommitSelectionMove(callback: (ctx: ReducerEventContext, projectId: string, offset: Vector3) => void) {
  ...
  modifyBlocks(projectId: string, mode: BlockModificationMode, minPos: Vector3, maxPos: Vector3, compressedVoxelData: Uint8Array, layerIndex: number) {
    const __args = { projectId, mode, minPos, maxPos, compressedVoxelData, layerIndex };
    let __writer = new __BinaryWriter(1024);
    ModifyBlocks.serialize(__writer, __args);
    let __argsBuffer = __writer.getBuffer();
    this.connection.callReducer("ModifyBlocks", __argsBuffer, this.setCallReducerFlags.modifyBlocksFlags);
  }

  onModifyBlocks(callback: (ctx: ReducerEventContext, projectId: string, mode: BlockModificationMode, minPos: Vector3, maxPos: Vector3, compressedVoxelData: Uint8Array, layerIndex: number) => void) {
    this.connection.onReducer("ModifyBlocks", callback);
  }

  removeOnModifyBlocks(callback: (ctx: ReducerEventContext, projectId: string, mode: BlockModificationMode, minPos: Vector3, maxPos: Vector3, compressedVoxelData: Uint8Array, layerIndex: number) => void) {
    this.connection.offReducer("ModifyBlocks", callback);
  }
...
export class SetReducerFlags {
  ...
  modifyBlocksFlags: __CallReducerFlags = 'FullUpdate';
  modifyBlocks(flags: __CallReducerFlags) {
    this.modifyBlocksFlags = flags;
  }
