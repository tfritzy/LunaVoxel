...
const loadSvgAsCursor = async (svgPath: string, scale: number = 0.8): Promise<string> => {
    try {
        const response = await fetch(svgPath);
        let svgContent = await response.text();

        // Wrap the entire SVG content in a scaled group
...
const getCursorConfig = (mode: BlockModificationMode): { path: string; hotspot: [number, number]; scale: number } => {
    switch (mode.tag) {
        case "Erase":
            return {
...
export const useCustomCursor = (mode: BlockModificationMode) => {
    ...
    useEffect(() => {
        const loadCursor = async () => {
            ...
            if (dataUrl) {
                setCursorStyle(`url('${dataUrl}') ${config.hotspot[0]} ${config.hotspot[1]}, auto`);
            } else {
                setCursorStyle("auto");
            }
        };
