...
import { downloadFile, downloadImageFromCanvas, extractTextureAtlasImage } from "./file-utils";
...
export class BlenderExporter {
    ...
    public exportOBJ(): void {
        try {
            ...
            const consolidator = new MeshConsolidator(this.chunkManager, worldDimensions);
            const consolidatedMesh = consolidator.consolidateAllChunks();

            if (consolidatedMesh.vertices.length === 0) {
                alert('No geometry to export. Please add some blocks to your project first.');
                return;
            }
            ...
            if (this.textureAtlas) {
                try {
                    ...
                    alert('3D model exported successfully, but texture export failed. You may need to manually apply textures in Blender.');
                }
            } else {
                alert('3D model exported successfully! Note: No texture atlas was available, so you may need to manually apply textures in Blender.');
            }

            console.log(`Successfully exported ${projectName} with ${consolidatedMesh.vertices.length / 3} vertices and ${consolidatedMesh.indices.length / 3} faces.`);
