...
public static partial class Module
{
    ...
    [Reducer]
    public static void ReplacePalette(ReducerContext ctx, string worldId, string[] colors)
    {
        if (colors.Length == 0)
        {
        ...
        var playersInWorld = ctx.Db.PlayerInWorld.world_color
            .Filter((worldId, ctx.Sender.ToString()))
            .ToList();
